...
Tài liệu này quy chuẩn hóa quy trình triển khai và phối hợp làm việc trên hệ thống quản lý hạ tầng bằng công cụ `Terraform` gắn chặt với nền tảng `GitHub Actions`. Quy trình được thiết kế chuyên biệt cho nhóm gồm hai Thành viên nhằm đảm bảo tính toàn vẹn, khả năng kiểm toán mã nguồn và mức độ an toàn khi làm việc nhằm ngăn ngừa sự cố khi can thiệp vào tài nguyên hạ tầng đám mây.
...
- **Nhánh `feature/*`**: Các nhánh vòng đời ngắn hạn (ephemeral branches), được phân tách từ nhánh `master`. Thành viên sử dụng nhánh này để phát triển và cập nhật cấu trúc hạ tầng độc lập.

## 3. Lịch trình triển khai cốt lõi

### Giai đoạn 1: Phát triển tính năng (feature development)

1. Thành viên chịu trách nhiệm tải (pull) mã nguồn phiên bản mới nhất từ nhánh `master` về môi trường cục bộ.
...
3. Cần lưu ý rằng, thành viên bắt buộc thực hiện việc căn chỉnh cấu hình Terraform cục bộ cho tới khi đáp ứng yêu cầu kỹ thuật.
4. Viết commit theo cú pháp chuẩn của [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/)

### Giai đoạn 2: Tích hợp liên tục (continuous integration - CI)

1. Thành viên hoàn tất phần mã nguồn tại nhánh `feature` chủ động khởi tạo một Pull Request - PR hướng về nhánh `dev`.
...
4. **Phê duyệt đối chiếu**: Thành viên còn lại có trách nhiệm xem xét, đánh giá rủi ro từ bản kế hoạch. Chỉ khi kết quả kiểm thử **báo xanh** và người bình duyệt chấp thuận, PR đính kèm mới được hợp nhất (merge) vào nhánh `dev`.

### Giai đoạn 3: Áp dụng môi trường hạ tầng (infrastructure apply)

1. **Kích hoạt Thủ công**: Việc mã nguồn được đưa vào nhánh `dev` không tự động thay đổi tài nguyên vật lý. Một Thành viên áp dụng môi trường, kích hoạt thủ công chu trình triển khai `Terraform Apply` trên Github Actions.
2. **Khai báo tham số thực thi**:
   - Tiến trình bắt buộc khởi chạy tại nhánh `dev`.
   - Thành viên cần điền trực tiếp mã băm (`plan_commit_sha`) được lưu trữ từ Giai đoạn 2 nhằm chỉ thị cho hệ thống tải về phiên bản chính xác của bản kế hoạch trực thuộc S3.
...
1. Thành viên bắt buộc kích hoạt tiến trình trực tiếp từ nhánh `dev`.
2. Hệ thống thiết lập rào cản nhân lực và yêu cầu thành viên gõ thủ công chuỗi ký tự chính xác "destroy". Nếu cung cấp sai tham số, hệ thống sẽ tự động bãi bỏ.
